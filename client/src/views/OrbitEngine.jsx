...
export default function OrbitEngine({ onPhysicsChange }) {
  // State management
  ...
  const connectWebSocket = () => {
    ...
    ws.onopen = () => {
      ...
      ws.send(
        JSON.stringify({ type: "control", action: "set_speed", speed: 1000 })
      );
    ...
    ws.onclose = () => {
      ...
      setLogs((prev) => [
        ...
        {
          type: "error",
          message: "WebSocket disconnected - reconnecting in 3s...",
        },
      ]);
  ...
  const handlePhysicsChange = (physicsType) => {
    ...
    console.log("Physics engine changed to:", physicsType);
    if (onPhysicsChange) {
